...
def get_links(company):
    ...
    all_tail_string = ['?lang=en'] + ['?start=' + str(i) + '&lang=en' for i in list(np.arange(1,200)*20)]
    all_link_lst = ['https://www.indeed.com/cmp/' + company + '/reviews' + i for i in all_tail_string]

    topic_tail_string = ['&lang=en'] + ['&start=' + str(i) + '&lang=en' for i in list(np.arange(1,60)*20)]
